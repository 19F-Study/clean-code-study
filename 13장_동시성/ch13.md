...
l provides a sufficient condition for correctness
n Classic solution
l Software C.S.: Peterson’s Solution l Hardware C.S.: Mutexes Locks
l Semaphores (Edsger Dijkstra)
...
- Thread Jiggling
  ...
  - Thread jiggler는 스레드의 "jiggling"명령들 사이에서 Thread.yield() 호출을 삽입하기 위해 런타임에 클래스 바이트 코드를 수정함으로써 작동한다.
